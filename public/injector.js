...
$( document ).ready(function() {
  ...
  var timerFunction2 = setTimeout(function(){
    ...
    $.getJSON("output.json", function (result) {
      ...
      htmltoappend = htmltoappend + '<tr><th>'+'Parkrun'+'</th><th>' + "Result" + '</th><th>' + 'Parkrunner' + '</th>' + '<th>' + 'Time' + '</th>' + '<th>' + 'Agecat' + '</th>' + '<th>' + 'Agegrade' + '<th class="gender">'+'Gender' + '</th>' +  '<th>' + 'GenderPos' + '</th>' + '<th>' + 'Note' + '</th>' + '<th>' + 'Total Runs' + '</th></tr>';
      htmltoappend = htmltoappend + '</thead><tbody>'
      for (i = 0; i < result.length; i++) {
        htmltoappend = htmltoappend + '<tr>' + '<td>' + result[i].parkrun.split(" ")[0] + '</td><td>' + result[i].pos + '</td><td>' + result[i].parkrunner + '</td>' + '<td>' + result[i].time + '</td>' + '<td>' + result[i].agecat + '</td>' + '<td>' + result[i].agegrade + '</td>' + '<td class="gender">' + result[i].gender + '</td>' + '<td>' + result[i].genderpos + '</td>' + '<td>' + result[i].Note + '</td>' + '<td>' + result[i].TotalRuns + '</th>' + '</tr>';
